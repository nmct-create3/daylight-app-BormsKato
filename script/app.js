...
let showResult = function (jsonObject) {
  ...
  if (hours > 0) {
    document.querySelector(
      '.js-time-left'
    ).innerHTML = `${hours} hours and ${minutes} minutes `;
  } else {
    document.querySelector('.js-time-left').innerHTML = `${minutes} minutes `;
  }

  const totalMinutes = Math.floor(
    (new Date(jsonObject.city.sunset) - new Date(jsonObject.city.sunrise)) / 60
  );
...
let getAPI = (lat, lon) => {
  ...
  handleData(
    `http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=f521e55c82e9611038d6dea79fc95a63&units=metric&lang=nl`,
    showResult
  );
};
